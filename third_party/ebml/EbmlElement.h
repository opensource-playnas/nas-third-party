...
*/
int CodedSizeLength(uint64 Length, unsigned int SizeLength, bool bSizeIsFinite = true);
...
#define DECLARE_EBML_MASTER(x)  class x : public EbmlMaster { \
  public: \
    x();
#define DECLARE_EBML_UINTEGER(x)  class x : public EbmlUInteger { \
  public: \
    x();
#define DECLARE_EBML_STRING(x)    class x : public EbmlString { \
  public: \
    x();
#define DECLARE_EBML_BINARY(x)    class x : public EbmlBinary { \
  public: \
    x();
