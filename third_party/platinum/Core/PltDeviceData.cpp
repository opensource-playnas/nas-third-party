...
PLT_DeviceData::AddService(PLT_Service* service)
{
    ...
    if (service->GetServiceType().Find("upnp", 0, true) == -1) {
        // 忽略非 upnp 的设备,
      // TODO(lxh): 由于小米盒子的设备服务中有一个 urn:mi-com:service:RController:1 类型的服务，但是该服务的scpd xml中没有action 列表，导致在检测服务是否准备好时一直check不过，所以暂时忽略掉非upnp的服务
        return NPT_SUCCESS;
    }
    UpdateConfigId();
