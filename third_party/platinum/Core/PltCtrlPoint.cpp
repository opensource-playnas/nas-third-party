...
class PLT_AddGetSCPDRequestIterator {
 public:
  ...
  NPT_Result operator()(PLT_Service*& service) const {
    ...
    NPT_LOG_FINER_3(
        "Queueing SCPD request for service \"%s\" of device \"%s\" @ %s",
    ...
    if (!url.IsValid()) {
      NPT_LOG_SEVERE_3(
          "Invalid SCPD url \"%s\" for service \"%s\" of device \"%s\"!",
          (const char*)scpd_url, (const char*)service->GetServiceID(),
          (const char*)service->GetDevice()->GetFriendlyName());
    ...
    PLT_CtrlPointGetSCPDRequest* request =
        new PLT_CtrlPointGetSCPDRequest((PLT_DeviceDataReference&)m_Device,
                                        scpd_url, "GET", NPT_HTTP_PROTOCOL_1_1);
 ...
 public:
  ...
  NPT_Result operator()(PLT_DeviceDataReference& device) const {
    NPT_Result res = device->m_Services.ApplyUntil(
        PLT_ServiceReadyIterator(), NPT_UntilResultNotEquals(NPT_SUCCESS));
    if (NPT_FAILED(res))
      return res;

    res = device->m_EmbeddedDevices.ApplyUntil(
        PLT_DeviceReadyIterator(), NPT_UntilResultNotEquals(NPT_SUCCESS));
    if (NPT_FAILED(res))
      return res;
...
+---------------------------------------------------------------------*/
PLT_CtrlPoint::PLT_CtrlPoint(
    const char* search_criteria /* = "upnp:rootdevice" */)
...
+---------------------------------------------------------------------*/
void PLT_CtrlPoint::IgnoreUUID(const char* uuid) {
...
|   PLT_CtrlPoint::Start
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::Start(PLT_SsdpListenTask* task) {
  if (m_Started)
  ...
  m_EventHttpServer->AddRequestHandler(new PLT_HttpRequestHandler(this), "/",
                                       true, true);
  ...
  // return m_SearchCriteria.GetLength()?Search(NPT_HttpUrl("239.255.255.250",
  // 1900, "*"), m_SearchCriteria, 1, 5000):NPT_SUCCESS;
  //

  m_Started = true;

  return m_SearchCriteria.GetLength()
             ? Search(NPT_HttpUrl("239.255.255.250", 1900, "*"),
                      m_SearchCriteria)
             : NPT_SUCCESS;
}
...
|   PLT_CtrlPoint::Stop
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::Stop(PLT_SsdpListenTask* task) {
  if (!m_Started)
    NPT_CHECK_WARNING(NPT_ERROR_INVALID_STATE);
  ...
  NPT_List<PLT_DeviceDataReference>::Iterator iter =
      m_RootDevices.GetFirstItem();
  while (iter) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::AddListener(PLT_CtrlPointListener* listener) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::RemoveListener(PLT_CtrlPointListener* listener) {
  NPT_AutoLock lock(m_Lock);
...
+---------------------------------------------------------------------*/
PLT_SsdpSearchTask* PLT_CtrlPoint::CreateSearchTask(
  ...
  // create socket
  NPT_Reference<NPT_UdpMulticastSocket> socket(
      new NPT_UdpMulticastSocket(NPT_SOCKET_FLAG_CANCELLABLE));
  socket->SetInterface(address);
  socket->SetTimeToLive(
      PLT_Constants::GetInstance().GetSearchMulticastTimeToLive());
  ...
  do {
    ...
    if (NPT_SUCCEEDED(
            socket->Bind(NPT_SocketAddress(NPT_IpAddress::Any, port), false)))
      break;
  ...
  NPT_HttpRequest* request =
      new NPT_HttpRequest(url, "M-SEARCH", NPT_HTTP_PROTOCOL_1_1);
  ...
  request->GetHeaders().SetHeader(
      NPT_HTTP_HEADER_USER_AGENT,
      *PLT_Constants::GetInstance().GetDefaultUserAgent());

  // create task
  PLT_SsdpSearchTask* task = new PLT_SsdpSearchTask(
      socket.AsPointer(), this, request,
      (frequency.ToMillis() > 0 && frequency.ToMillis() < 5000)
          ? NPT_TimeInterval(5.)
          : frequency); /* repeat no less than every 5 secs */
  socket.Detach();
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::Search(
    const NPT_HttpUrl& url,
    const char* target,
    NPT_Cardinal mx /* = 5 */,
  ...
  for (net_if = if_list.GetFirstItem(); net_if; net_if++) {
    ...
    for (net_if_addr = (*net_if)->GetAddresses().GetFirstItem(); net_if_addr;
         net_if_addr++) {
      // create task
      PLT_SsdpSearchTask* task = CreateSearchTask(
          url, target, mx, frequency, (*net_if_addr).GetPrimaryAddress());
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::Discover(
    const NPT_HttpUrl& url,
    const char* target,
    NPT_Cardinal mx, /* = 5 */
    NPT_TimeInterval frequency /* = NPT_TimeInterval(50.) */,
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::DoHouseKeeping() {
  ...
  // remove expired devices
  {
    ...
    while (NPT_SUCCEEDED(m_RootDevices.PopHead(device))) {
      ...
      // check if device lease time has expired or if failed to renew
      // subscribers
      // TODO: UDA 1.1 says that root device and all embedded devices must have
      // expired before we can assume they're all no longer unavailable (we may
      // have missed the root device renew)
      NPT_TimeStamp now;
      NPT_System::GetCurrentTimeStamp(now);
      if (now > last_update + lease_time) {
...
#if 0  //  TODO(lxh): 在设备频繁断线的情况下，sub中的service
       //  对象会被析构，但是队列里面偶尔还存在，会导致崩溃
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::FindDevice(const char* uuid,
                                     PLT_DeviceDataReference& device,
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::FindActionDesc(PLT_DeviceDataReference& device,
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::CreateAction(PLT_DeviceDataReference& device,
                                       const char* service_type,
                                       const char* action_name,
                                       PLT_ActionReference& action) {
  if (device.IsNull())
  ...
  PLT_ActionDesc* action_desc;
  NPT_CHECK_SEVERE(
      FindActionDesc(device, service_type, action_name, action_desc));
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::SetupResponse(NPT_HttpRequest& request,
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::DecomposeLastChangeVar(
    NPT_List<PLT_StateVariable*>& vars) {
  // parse LastChange var into smaller vars
  PLT_StateVariable* lastChangeVar = NULL;
  if (NPT_SUCCEEDED(NPT_ContainerFind(
          vars, PLT_StateVariableNameFinder("LastChange"), lastChangeVar))) {
    ...
    if (!node->GetTag().Compare("Event", true)) {
      ...
      if (instance != NULL) {
        ...
        for (NPT_Cardinal j = 0; j < instance->GetChildren().GetItemCount();
             j++) {
          ...
          PLT_StateVariable* var =
              var_service->FindStateVariable(var_node->GetTag());
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessEventNotification(
  ...
  // so we disregard it
  if (subscriber->GetEventKey() &&
      notification->m_EventKey < subscriber->GetEventKey()) {
    NPT_CHECK_LABEL_WARNING(NPT_FAILURE, failure);
  }
  ...
  PLT_StateVariable* var;
  for (NPT_List<NPT_XmlNode*>::Iterator children =
           xml->GetChildren().GetFirstItem();
       children; children++) {
    ...
    if (NPT_FAILED(
            var->SetValue(property->GetText() ? *property->GetText() : ""))) {
      NPT_CHECK_LABEL_WARNING(NPT_FAILURE, failure);
    }
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::AddPendingEventNotification(
    PLT_EventNotification* notification) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessPendingEventNotifications() {
  ...
  while (count--) {
    ...
    if (NPT_SUCCEEDED(m_PendingNotifications.PopHead(notification))) {
      ...
      if (NPT_FAILED(NPT_ContainerFind(
              m_Subscribers,
              PLT_EventSubscriberFinderBySID(notification->m_SID), sub))) {
        m_PendingNotifications.Add(notification);
        continue;
      }
      ...
      NPT_LOG_WARNING_1("Reprocessing delayed notification for subscriber %s",
    ...
    if (service && vars.GetItemCount()) {
      m_ListenerList.Apply(
          PLT_CtrlPointListenerOnEventNotifyIterator(service, &vars));
    }
  }
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessHttpNotify(
  ...
  PLT_LOG_HTTP_REQUEST(NPT_LOG_LEVEL_FINER,
                       "PLT_CtrlPoint::ProcessHttpNotify:", &request);

  // Create notification from request
  PLT_EventNotification* notification =
      PLT_EventNotification::Parse(request, context, response);
  NPT_CHECK_POINTER_LABEL_WARNING(notification, bad_request);

  // Give a last change to process pending notifications before throwing them
  // out by AddPendingNotification
  ProcessPendingEventNotifications();

  // look for the subscriber with that sid
  if (NPT_FAILED(NPT_ContainerFind(
          m_Subscribers, PLT_EventSubscriberFinderBySID(notification->m_SID),
          sub))) {
    NPT_LOG_WARNING_1(
        "Subscriber %s not found, delaying notification process.\n",
        (const char*)notification->m_SID);
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessSsdpSearchResponse(
    NPT_Result res,
    const NPT_HttpRequestContext& context,
    NPT_HttpResponse* response) {
  ...
  // NPT_IpAddress dangbei;
  //dangbei.Parse("172.16.10.34");


  // auto remote_ip = context.GetRemoteAddress().GetIpAddress();

  //if (remote_ip == dangbei) {
  //  //printf("ProcessSsdpNotify: %s\n", remote_ip.ToString().GetChars());
  //} else {
  //  return NPT_FAILURE;
  //}
  ...
  NPT_String prefix = NPT_String::Format(
      "PLT_CtrlPoint::ProcessSsdpSearchResponse from %s:%d",
  ...
  if (response->GetStatusCode() / 100 == 2) {
    ...
    if (m_UUIDsToIgnore.Find(NPT_StringFinder(uuid))) {
      NPT_LOG_FINE_1(
          "CtrlPoint received a search response from ourselves (%s)\n",
          (const char*)uuid);
      return NPT_SUCCESS;
    }
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::OnSsdpPacket(const NPT_HttpRequest& request,
                                       const NPT_HttpRequestContext& context) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessSsdpNotify(
    const NPT_HttpRequest& request,
    const NPT_HttpRequestContext& context) {
  ...
  if (method.Compare("NOTIFY") == 0) {
    ...
    NPT_String prefix = NPT_String::Format(
        "PLT_CtrlPoint::ProcessSsdpNotify from %s:%d (%s)",
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::AddDevice(PLT_DeviceDataReference& data) {
  NPT_AutoLock lock(m_Lock);
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::NotifyDeviceReady(PLT_DeviceDataReference& data) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::RemoveDevice(PLT_DeviceDataReference& data) {
  NPT_AutoLock lock(m_Lock);
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::NotifyDeviceRemoved(PLT_DeviceDataReference& data) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::CleanupDevice(PLT_DeviceDataReference& data) {
  if (data.IsNull())
    return NPT_ERROR_INVALID_PARAMETERS;
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessSsdpMessage(
  ...
  // be nice and assume a default lease time if not found even though it's
  // required
  ...
  if (NPT_SUCCEEDED(FindDevice(uuid, data))) {
    //        // in case we missed the byebye and the device description has
    //        changed (ip or port)
    //        // reset base and assumes device is the same (same number of
    //        services and embedded devices)
    //        // FIXME: The right way is to remove the device and rescan it
    //        though but how do we know it changed? PLT_DeviceReadyIterator
    //        device_tester; if (NPT_SUCCEEDED(device_tester(data)) &&
    //        data->GetDescriptionUrl().Compare(location.ToString(), true)) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::InspectDevice(const NPT_HttpUrl& location,
                                        const char* uuid,
                                        NPT_TimeInterval leasetime) {
  NPT_AutoLock lock(m_Lock);
  ...
  if (NPT_SUCCEEDED(NPT_ContainerFind(m_PendingInspections,
                                      NPT_StringFinder(uuid), pending_uuid))) {
    return NPT_SUCCESS;
  }

  NPT_LOG_INFO_2("Inspecting device \"%s\" detected @ %s", uuid,
                 (const char*)location.ToString());
  ...
  PLT_CtrlPointGetDescriptionTask* task =
      new PLT_CtrlPointGetDescriptionTask(location, this, leasetime, uuid);
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::FetchDeviceSCPDs(PLT_CtrlPointGetSCPDsTask* task,
                                           PLT_DeviceDataReference& device,
                                           NPT_Cardinal level) {
  ...
  for (NPT_Cardinal i = 0; i < device->m_EmbeddedDevices.GetItemCount(); i++) {
    NPT_CHECK_SEVERE(
        FetchDeviceSCPDs(task, device->m_EmbeddedDevices[i], level));
  }
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessGetDescriptionResponse(
    NPT_Result res,
    const NPT_HttpRequest& request,
    const NPT_HttpRequestContext& context,
    NPT_HttpResponse* response,
  ...
  NPT_String prefix = NPT_String::Format(
      "PLT_CtrlPoint::ProcessGetDescriptionResponse @ %s (result = %d, status "
      "= %d)",
      (const char*)request.GetUrl().ToString(), res,
  ...
  NPT_CHECK_SEVERE(PLT_DeviceData::SetDescription(
      root_device, leasetime, request.GetUrl(), desc, context));
...
bad_response:
  NPT_LOG_SEVERE_2("Bad Description response @ %s: %s",
                   (const char*)request.GetUrl().ToString(), (const char*)desc);
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ProcessGetSCPDResponse(
    NPT_Result res,
    const NPT_HttpRequest& request,
    const NPT_HttpRequestContext& context,
    NPT_HttpResponse* response,
  ...
  NPT_String prefix = NPT_String::Format(
      "PLT_CtrlPoint::ProcessGetSCPDResponse for a service of device \"%s\" @ "
      "%s (result = %d, status = %d)",
  ...
  res =
      device->FindServiceBySCPDURL(request.GetUrl().ToRequestString(), service);
  ...
  // DIAL support
  if (root_device->GetType().Compare(
          "urn:dial-multiscreen-org:device:dial:1") == 0) {
...
bad_response:
  NPT_LOG_SEVERE_2("Bad SCPD response for device \"%s\":%s",
                   (const char*)device->GetFriendlyName(), (const char*)scpd);
...
+---------------------------------------------------------------------*/
PLT_ThreadTask* PLT_CtrlPoint::RenewSubscriber(
    PLT_EventSubscriberReference subscriber) {
  NPT_AutoLock lock(m_Lock);
  ...
  NPT_LOG_FINE_3(
      "Renewing subscriber \"%s\" for service \"%s\" of device \"%s\"",
  ...
  // create the request
  NPT_HttpRequest* request =
      new NPT_HttpRequest(subscriber->GetService()->GetEventSubURL(true),
  ...
  PLT_UPnPMessageHelper::SetTimeOut(*request,
                                    (NPT_Int32)PLT_Constants::GetInstance()
                                        .GetDefaultSubscribeLease()
                                        ->ToSeconds());

  // Prepare the request
  // create a task to post the request
  return new PLT_CtrlPointSubscribeEventTask(request, this, root_device,
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::Subscribe(PLT_Service* service,
                                    bool cancel,
                                    void* userdata) {
  NPT_AutoLock lock(m_Lock);
  ...
  PLT_EventSubscriberReference sub;
  NPT_CHECK_WARNING(
      FindDevice(service->GetDevice()->GetUUID(), root_device, true));

  // look for the subscriber with that service to decide if it's a renewal or
  // not
  NPT_ContainerFind(m_Subscribers, PLT_EventSubscriberFinderByService(service),
                    sub);
  ...
  if (cancel == false) {
    ...
    NPT_HttpUrl callbackUrl(service->GetDevice()->m_LocalIfaceIp.ToString(),
                            m_EventHttpServer->GetPort(), callback_uri);
    ...
    auto sub_timeout = (NPT_Int32) PLT_Constants::GetInstance()
        .GetDefaultSubscribeLease()
        ->ToSeconds();
    PLT_UPnPMessageHelper::SetTimeOut(*request, sub_timeout);
  } else {
    NPT_LOG_INFO_3(
        "Unsubscribing subscriber \"%s\" for service \"%s\" of device \"%s\"",
  ...
  PLT_ThreadTask* task = new PLT_CtrlPointSubscribeEventTask(
      request, this, root_device, service, userdata);
...
NPT_Result PLT_CtrlPoint::ProcessSubscribeResponse(
  ...
  NPT_String prefix = NPT_String::Format(
      "PLT_CtrlPoint::ProcessSubscribeResponse %ubscribe for service \"%s\" "
      "(result = %d, status code = %d)",
      (const char*)subscription ? "S" : "Uns",
      (const char*)service->GetServiceID(), res,
      response ? response->GetStatusCode() : 0);
  PLT_LOG_HTTP_RESPONSE(NPT_LOG_LEVEL_FINER, prefix, response);
  ...
  if (subscription) {
    ...
    // Look for subscriber
    NPT_ContainerFind(m_Subscribers, PLT_EventSubscriberFinderBySID(*sid), sub);

    NPT_LOG_INFO_5(
        "%s subscriber \"%s\" for service \"%s\" of device \"%s\" (timeout = "
        "%d)",
        !sub.IsNull() ? "Updating timeout for" : "Creating new",
        (const char*)*sid, (const char*)service->GetServiceID(),
        (const char*)service->GetDevice()->GetFriendlyName(), seconds);
    ...
    // Process any pending notifcations for that subscriber we got a bit too
    // early
    ProcessPendingEventNotifications();
...
remove_sub:
  // in case it was a renewal look for the subscriber with that service and
  // remove it from the list
  if (NPT_SUCCEEDED(NPT_ContainerFind(
          m_Subscribers, PLT_EventSubscriberFinderByService(service), sub))) {
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::InvokeAction(PLT_ActionReference& action,
                                       void* userdata) {
  if (!m_Started)
    NPT_CHECK_WARNING(NPT_ERROR_INVALID_STATE);
  ...
  NPT_HttpRequest* request =
      new NPT_HttpRequest(url, "POST", NPT_HTTP_PROTOCOL_1_1);
  ...
  PLT_CtrlPointInvokeActionTask* task =
      new PLT_CtrlPointInvokeActionTask(request, this, action, userdata);
...
NPT_Result PLT_CtrlPoint::ProcessActionResponse(
  ...
  if (NPT_FAILED(res) || response == NULL) {
    ...
    NPT_LOG_WARNING_4("Failed to reach %s for %s.%s (%d)",
                      ...
                      service->GetServiceName().GetChars(), res);
    goto failure;
  }

  PLT_LOG_HTTP_RESPONSE(NPT_LOG_LEVEL_FINER,
  ...
  // check namespace
  if (!xml->GetNamespace() ||
      xml->GetNamespace()->Compare("http://schemas.xmlsoap.org/soap/envelope/"))
    goto failure;

  // check encoding
  attr = xml->GetAttribute("encodingStyle",
  ...
  if (soap_action_response->GetTag().Compare(action_desc.GetName() + "Response",
                                             true))
    goto failure;

  // verify namespace
  if (!soap_action_response->GetNamespace() ||
      soap_action_response->GetNamespace()->Compare(
          action_desc.GetService()->GetServiceType()))
    goto failure;

  // read all the arguments if any
  for (NPT_List<NPT_XmlNode*>::Iterator args =
           soap_action_response->GetChildren().GetFirstItem();
       args; args++) {
...
cleanup : {
  NPT_AutoLock lock(m_Lock);
  m_ListenerList.Apply(
      PLT_CtrlPointListenerOnActionResponseIterator(res, action, userdata));
}
...
+---------------------------------------------------------------------*/
NPT_Result PLT_CtrlPoint::ParseFault(PLT_ActionReference& action,
                                     NPT_XmlElementNode* fault) {
