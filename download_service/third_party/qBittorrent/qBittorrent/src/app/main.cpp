...
int main(int argc, char *argv[])
{
    ...
    g_sem_stop.release();
    qDebug() << "quit main";
    LogMsg(QString::fromUtf8("quit main"), Log::INFO);
    return ret;
}
...
extern "C" Q_DECL_EXPORT TorrentFileDownloadTask* CreateTorrentFileTask() {
    TorrentFileDownloadTask* task = new TorrentlFileDownloadTaskImpl();
    return task;
}

extern "C" Q_DECL_EXPORT MagnetDownloadTask* CreateMagnetTask() {
    MagnetDownloadTask* task = new MagnetDownloadTaskImpl();
    return task;
}


extern "C" Q_DECL_EXPORT BtSettingInterface* CreateBtSettings() {
    TorrentSettings * settings = new TorrentSettings();
    return settings;
}

extern "C" Q_DECL_EXPORT void ReleaseTask(DownloadTaskInterface* task) {
    if (task) {
        delete task;
        task = NULL;
    }
...
extern "C" Q_DECL_EXPORT int Init(int argc, char *argv[]) {
  g_thread = new std::thread(main, argc, argv);
  return true;
}

extern "C" Q_DECL_EXPORT int UnInit() {
  BaseApplication::quit();
  g_sem_stop.tryAcquire(1, 30000);
  return 0;
}

extern "C" Q_DECL_EXPORT int WaitInitFinish() {
    g_sem.acquire();
    qDebug() << "WaitInitFinish";
    return 1;
}
