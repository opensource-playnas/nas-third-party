...
extern "C" WXEXPORT MuleDownloadTask *CreateTask() {
	MuleDownloadTask*task = new MuleDownloadTaskImpl();
  return task;
}

std::thread *g_thread = NULL;
extern "C" WXEXPORT int __cdecl Init(int argc, char *argv[]) {
  g_thread = new std::thread(main, argc, argv);
  return true;
}

extern "C" WXEXPORT int __cdecl UnInit() { 
	theApp->ExitMainLoop();
	if (g_thread) {
		g_thread->join();
	}
...
extern "C" WXEXPORT int __cdecl WaitInitFinish() {
	g_sem.Wait();
  return 1;
}

extern "C" WXEXPORT void ReleaseTask(DownloadTaskInterface *task) {
  if (task) {
    delete task;
    task = NULL;
  }
...
extern "C" WXEXPORT AmuleSettingInterface * CreateAuleSettings() {
	AmuleSettings* g_settings = new AmuleSettings();
  return g_settings;
}
