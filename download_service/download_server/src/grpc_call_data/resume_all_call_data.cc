#include "resume_all_call_data.h"
#include "nas/download_service/download_server/src/download_mgr.h"


namespace nas {

ResumeAllCallData::ResumeAllCallData(
    nas::DownloadServiceImpl* service,
    ServerCompletionQueue* cq,
    NasThread* cq_dispatch,
    NasThread* task_main)
    : nas::GrpcServerCallBase<nas::DownloadServiceImpl,
                              ResumeAllCallData>(service,
                                                              cq,
                                                              cq_dispatch,
                                                              task_main),
      responder_(&ctx_) {}

void ResumeAllCallData::Request(
    DownloadServiceImpl* service,
    ServerCompletionQueue* cq) {
  if (service && cq && tag_) {
    service->RequestResumeAll(&ctx_, &request_, &responder_, cq,
                                           cq, tag_);
  }
}

void ResumeAllCallData::Process() {
  ...
  if (service_) {
    ...
    if (download_mgr) {
      download_mgr->ResumeAll(context_field);
    }
  }
